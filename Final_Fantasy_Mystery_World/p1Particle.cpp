...
//Constructor
p1Particle::p1Particle(fPoint respawn, iPoint area, iPoint timelife, fPoint speed_particle, P_Direction p_direction, SDL_Rect pos_rect, int size, int num_tex_pixel, bool active, Wind dir, iPoint num_tex)
{
...
bool p1Particle::Modify(fPoint respawn, iPoint area, iPoint timelife, iPoint num_tex_p)
{
	//with random change position
	if (area.x == 0)
		position.x = respawn.x;
...
bool p1Particle::isDead()
{
	//return is dead;
	return degrade.y == 0;
}

void p1Particle::render()
{
...
fPoint p1Particle::GetPosition()
{
	return position;
}

fPoint p1Particle::GetSpeed()
{
	return speed;
}

void p1Particle::SetSpeedGreavity(fPoint speed_modify)
{
	speed.x += speed_modify.x;
	speed.y += speed_modify.y;
}

void p1Particle::SetRepeat(bool temp)
{
	not_repeat = temp;
}

bool p1Particle::GetRepeat()
{
	return not_repeat;
}
